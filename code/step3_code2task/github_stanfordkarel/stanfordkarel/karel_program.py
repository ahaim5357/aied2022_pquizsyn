...
class KarelProgram:
    def __init__(self, world_file: str, type:str) -> None:
        """
        ...
        self.type = type
        if self.type == "karel" or self.type == "hoc":
            pass
        else:
            assert "Unknown task type encountered!"
            exit(0)
        ...
        ### NOT PRESENT IN ORIGINAL REPO: ADDITONALLY ADDED PAPARMATERS
        self.karel_seq = []
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, KarelProgram):
            if self.type == "hoc":
                flag = (self.direction==other.direction) or (self.direction == Direction.UNK) or (other.direction == Direction.UNK)
            else:
                flag = self.direction == other.direction

            #### Useful for debugging:
            # print("Worlds match:", self.world == other.world)
            # print("Avenue Match:", self.avenue==other.avenue)
            # print("Street Match:", self.street == other.street)
            # print("Direction Match:", flag, self.direction, other.direction)
            # print("Beeper Match:", self.num_beepers==other.num_beepers)
            # print("Karel sequence:", self.karel_seq, other.karel_seq)

            return self.world == other.world and self.avenue==other.avenue and self.street==other.street and \
        flag and self.num_beepers==other.num_beepers
    ...
    ####### ADDITONALLY ADDED: NOT PRESENT IN ORIGINAL REPO
    ...
    def put_beeper(self) -> None:
        ...
        #### ADDED ADDITONALLY: NOT PRESENT BEFORE. THIS IS DONE TO ONLY ALLOW ONE BEEPER IN A LOCATION.
        if self.beepers_present():
            raise KarelException(
                self.avenue,
                self.street,
                self.direction,
                "Karel attempted to put multiple beepers on the current corner.",
            )
